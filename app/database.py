...
def get_db_raw():
    """Legacy: returns a raw connection (caller must close)."""
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    conn.execute('PRAGMA foreign_keys = ON')
    return conn
...
def seed_initial_data(conn):
    ...
    cursor.execute("""
        ...
        VALUES (1, 'Moustapha', 'MO', 'XOF', 385000, 650000, 8, ?)
    ...
    cursor.execute("""
        ...
        VALUES (?, 'Tontine Cercle Proches', 'Cotisation mensuelle entre proches pour financer les projets de chacun.', 50000, 'XOF', 'monthly', '2026-07-01', 'organizer', 2, 'active')
    """, (t1_id,))

    members_t1 = [
        ('m-1', t1_id, 'Moustapha (Moi)', 'MO', '+221770000001', 1, 100, 3),
        ('m-2', t1_id, 'Ibrahim Diop', 'ID', '+221770000002', 0, 95, 1),
        ('m-3', t1_id, 'Fatou Ndiaye', 'FN', '+221770000003', 0, 100, 2),
        ('m-4', t1_id, 'Amadou Sow', 'AS', '+221770000004', 0, 90, 4),
        ('m-5', t1_id, 'Aissatou Ba', 'AB', '+221770000005', 0, 100, 5),
    ]
    cursor.executemany("""
    ...
    payments_t1 = [
        ...
        ('c-1', 'm-2', 1, '2026-07-05', 50000, 'Especes'),
        ...
        ('c-2', 'm-3', 1, '2026-08-05', 50000, 'Especes'),
    ...
    txs = [
        ...
        ('tx-2', 'tontine_payment', 'Cotisation Tontine', 50000, '2026-09-01', 'Cotisation Tour 3 - Tontine Cercle Proches', t1_id, None),
        ('tx-3', 'expense', 'Alimentation & Courses', 40000, '2026-09-02', 'Courses supermarche & marche', None, None),
        ('tx-4', 'savings_deposit', 'Epargne Projet', 35000, '2026-09-02', 'Ajout tirelire PC Travail', None, 'goal-1'),
    ...
    goals = [
        ('goal-1', 'Materiel PC / Setup Travail', 500000, 320000, 'equipement', '2026-11-30', 'PC', 'from-blue-500 to-indigo-600'),
        ('goal-2', 'Fonds de Securite & Urgences', 300000, 180000, 'urgence', '2026-12-31', 'FU', 'from-emerald-500 to-teal-600'),
        ('goal-3', 'Financement Formation & Certifs', 150000, 65000, 'formation', '2026-10-15', 'FC', 'from-amber-500 to-orange-600'),
    ]
    cursor.executemany("""
